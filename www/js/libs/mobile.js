...
get_establishment = function(establishment, cb) {
  ...
  $.get(url("user/api/" + establishment + "/establishment.json"))
  .complete(function(data){
  	cb(data);
  });
...
like_establishment = function(establishment) {
  return $.get(url("user/api/" + establishment + "/like.json"), function(data) {
    return console.log(data);
  });
};
