...
if (!window.FB) {
	window.FB = {
		...
		getDomain : function(domain, noForcedHTTPS) {
			var forceHTTPS = !noForcedHTTPS && (window.location.protocol == 'https:' || FB._https);
			switch (domain) {
				case 'api':
...
FB.provide('QS', {
	 ...
	 */
	decode : function(str) {
		var decode = decodeURIComponent, params = {}, parts = str.split('&'), i, pair;
...
FB.provide('Content', {
	...
	insertIframe : function(opts) {
		...
		if (document.attachEvent) {
			...
			var html = ('<iframe' + ' id="' + opts.id + '"' + ' name="' + opts.name + '"' + (opts.title ? ' title="' + opts.title + '"' : '') + (opts.className ? ' class="' + opts.className + '"' : '') + ' style="border:none;' + (opts.width ? 'width:' + opts.width + 'px;' : '') + (opts.height ? 'height:' + opts.height + 'px;' : '') + '"' + ' src="javascript:false;"' + ' frameborder="0"' + ' scrolling="no"' + ' allowtransparency="true"' + ' onload="FB.Content._callbacks.' + guid + '()"' + '></iframe>'
			);
...
FB.provide('Flash', {
	...
	embedSWF : function(name, swf, flashvars) {
		// create the swf
		var IE = !!document.attachEvent, html = ('<object ' + 'type="application/x-shockwave-flash" ' + 'id="' + name + '" ' + ( flashvars ? 'flashvars="' + flashvars + '" ' : '') + ( IE ? 'name="' + name + '" ' : '') + ( IE ? '' : 'data="' + swf + '" ') + ( IE ? 'classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" ' : ''
		) + 'allowscriptaccess="always">' + '<param name="movie" value="' + swf + '"></param>' + '<param name="allowscriptaccess" value="always"></param>' + '</object>'
		);
	...
	hasMinVersion : function() {
		if ( typeof FB.Flash._hasMinVersion === 'undefined') {
			var versionString, i, l, version = [];
			try {
				versionString = new ActiveXObject('ShockwaveFlash.ShockwaveFlash').GetVariable('$version');
			} catch(x) {
				if (navigator.mimeTypes.length > 0) {
					var mimeType = 'application/x-shockwave-flash';
					if (navigator.mimeTypes[mimeType].enabledPlugin) {
						var name = 'Shockwave Flash';
						versionString = (navigator.plugins[name + ' 2.0'] || navigator.plugins[name]).description;
					}
				}
			}
...
FB.provide('ApiServer', {
	 ...
	 */
	graph : function() {
		var args = Array.prototype.slice.call(arguments), atoms = args.shift().match(/\/?([^?]*)\??([^#]*)/), path = atoms[1], next = args.shift(), method, params, cb;
	...
	oauthRequest : function(domain, path, method, params, cb) {
		...
		params.sdk = 'joey';
		params.pretty = 0;
		// browser's default to pretty=1, explicitly setting to
		...
		var oldCb = cb;
		cb = function(response) {
			if (FB.Auth && response && FB.getAccessToken() == params.access_token && (response.error_code === '190' || (response.error && (response.error === 'invalid_token' || response.error.type === 'OAuthException')))) {
		...
		try {
			...
			try {
				if (!FB.initSitevars.corsKillSwitch && FB.ApiServer.corsPost(domain, path, method, FB.JSON.flatten(params), cb)) {
					return;
				}
			} catch (e2_ignore) {
	...
	jsonp : function(domain, path, method, params, cb) {
		var g = FB.guid(), script = document.createElement('script');
		...
		var url = (FB.getDomain(domain) + path + (path.indexOf('?') > -1 ? '&' : '?') + FB.QS.encode(params)
		);
		if (url.length > 2000) {
	...
	flash : function(domain, path, method, params, cb) {
		...
		FB.Flash.onReady(function() {
			...
			var url = FB.getDomain(domain) + path, body = FB.QS.encode(params);
			...
			// fire the request
			var reqId = document.XdComm.sendXdHttpRequest(method.toUpperCase(), url, body, null);
...
FB.provide('XD', {
	...
	init : function(channelUrl) {
		...
		var postMessageOrigin = (window.location.protocol + '//' + window.location.host + '/' + FB.guid());
		...
		if (window.addEventListener && !window.attachEvent && window.postMessage) {
			...
			if (document.getElementById('fb-root')) {
				...
				FB.XD._origin = (window.location.protocol + '//' + domain + '/' + FB.guid());
				FB.XD.Flash.init();
	...
	resolveRelation : function(id) {
		var pt, matches, parts = id.split('.'), node = window;
	...
	handler : function(cb, relation, forever, id, force_https) {
		...
		var xdProxy = FB.getDomain(( force_https ? 'https_' : '') + 'cdn') + FB.XD._xdProxyUrl + '#';
		id = id || FB.guid();
		...
		if (FB.XD._transport == 'fragment') {
			...
			xdProxy += ((xdProxy.indexOf('?') < 0 ? '?' : '&') + FB.XD.Fragment._magic + '#?=&'
			);
		}
	...
	PostMessage : {
		...
		init : function() {
			if (!FB.XD.PostMessage._isInitialized) {
				...
				window.addEventListener ? window.addEventListener('message', H, false) : window.attachEvent('onmessage', H);
	...
	Fragment : {
		...
		checkAndDispatch : function() {
			var loc = window.location.toString(), fragment = loc.substr(loc.indexOf('#') + 1), magicIndex = loc.indexOf(FB.XD.Fragment._magic);
			...
			if (magicIndex > 0) {
				...
				FB.XD.resolveRelation(FB.QS.decode(fragment).relation).FB.XD.recv(fragment);
			}
		}
	}
});
...
FB.provide('UA', {
	...
	_populate : function() {
		...
		FB.UA._mobile = ios || FB.UA._android || navigator.userAgent.match(/Mobile/i);
...
FB.provide('Arbiter', {
	...
	inform : function(method, params, relation, https, behavior) {
		...
		https |= (window != window.parent && document.referrer.indexOf('https:') === 0);

		// fallback static file based transport
		var url = (FB.getDomain(( https ? 'https_' : '') + 'staticfb', true) + FB.Arbiter._canvasProxyUrl + '#' + FB.QS.encode({
			method : method,
...
FB.provide('Canvas', {
	...
	_hideFlashCallback : function(params) {
		...
		for (var i = 0; i < candidates.length; i++) {
			...
			if (elem.type.toLowerCase() != "application/x-shockwave-flash" && elem.classid.toUpperCase() != FB.Canvas._flashClassID) {
				continue;
			}

			var good = false;
			for (var j = 0; j < elem.childNodes.length; j++) {
				if (elem.childNodes[j].nodeName.toLowerCase() == "param" && elem.childNodes[j].name.toLowerCase() == "wmode") {
					if (elem.childNodes[j].value.toLowerCase() == "opaque" || elem.childNodes[j].value.toLowerCase() == "transparent") {
						good = true;
					}
				}
			}
	...
	init : function() {
		...
		var channelUrl = FB.XD.handler(FB.Canvas._hideFlashCallback, 'top.frames[' + window.name + ']', true);
	...
	setSize : function(params) {
		...
		if (FB.Canvas._lastSize[params.frame]) {
			...
			if (FB.Canvas._lastSize[params.frame].width == params.width && (dHeight <= minGrow && dHeight >= -minShrink)) {
				return false;
			}
		}
	...
	setAutoGrow : function(onOrOff, interval) {
		...
		if (onOrOff === undefined || onOrOff) {
			if (FB.Canvas._timer === null) {
				FB.Canvas._timer = window.setInterval(FB.Canvas.setSize, interval || 100);
	...
	setUrlHandler : function(callback) {
		var channelUrl = FB.XD.handler(callback, 'top.frames[' + window.name + ']', true);
	...
	_passAppTtiMessage : function(callback, message_name) {
		...
		if (callback) {
			devCallback = FB.XD.handler(callback, 'top.frames[' + window.name + ']', false);
		}
	...
	_computeContentSize : function() {
		var body = document.body, docElement = document.documentElement, right = 0, bodyTop = Math.max(body.offsetTop, 0), docTop = Math.max(docElement.offsetTop, 0), bodyScroll = body.scrollHeight + bodyTop, bodyOffset = body.offsetHeight + bodyTop, docScroll = docElement.scrollHeight + docTop, docOffset = docElement.offsetHeight + docTop;
...
FB.provide('String', {
	...
	format : function(format) {
		...
		return format.replace(FB.String.format._formatRE, function(str, m) {
			var index = parseInt(m.substr(1), 10), value = values[index + 1];
	...
	quote : function(value) {
		var quotes = /["\\\x00-\x1f\x7f-\x9f]/g, subst = {// table of character substitutions
			'\b' : '\\b',
			'\t' : '\\t',
			'\n' : '\\n',
			'\f' : '\\f',
		...
		return quotes.test(value) ? '"' + value.replace(quotes, function(a) {
			var c = subst[a];
			if (c) {
				return c;
			}
...
FB.provide('Dom', {
	...
	removeCss : function(dom, className) {
		if (FB.Dom.containsCss(dom, className)) {
			...
			FB.Dom.removeCss(dom, className);
	...
	getByClass : function(className, dom, tagName) {
		...
		if (dom.querySelectorAll) {
			return FB.Array.toArray(dom.querySelectorAll(tagName + '.' + className));
		}
		var all = dom.getElementsByTagName(tagName), els = [];
	...
	getStyle : function(dom, styleProp) {
		...
		if (dom.currentStyle) {// camelCase (e.g. 'marginTop')
			...
			if (window.getComputedStyle) {
				y = document.defaultView.getComputedStyle(dom, null).getPropertyValue(styleProp);
				// special handling for IE
				// for some reason it doesn't return '0%' for defaults. so needed to
				// translate 'top' and 'left' into '0px'
				if (styleProp == 'background-position-y' || styleProp == 'background-position-x') {
					if (y == 'top' || y == 'left') {
						y = '0px';
					}
				}
...
FB.provide('Intl', (function() {
	 ...
	 */
	var _punctCharClass = ('[' + '.!?' + '\u3002' + // Chinese/Japanese period
	...
	function _endsInPunct(str) {
		...
		return str.match(new RegExp(_punctCharClass + '[' + ')"' + "'" +
	...
	function _substituteTokens(str, args) {
		...
		if (args !== undefined) {
			if ( typeof args != 'object') {
				FB.log('The second arg to FB.Intl.tx() must be an Object for ' + 'FB.Intl.tx(' + str + ', ...)');
			} else {
				var regexp;
				for (var key in args) {
					if (args.hasOwnProperty(key)) {
						...
						if (_endsInPunct(args[key])) {
							...
							// after it, if any.
							regexp = new RegExp('\{' + key + '\}' + _punctCharClass + '*', 'g');
						} else {
...
FB.provide('', {
	...
	subclass : function(name, baseName, constructor, proto) {
		...
		return FB.Class(name, constructor ? constructor : function() {
			if (base.apply) {
...
FB.provide('Dialog', {
	...
	_createMobileLoader : function() {
		...
		var chrome = FB.UA.nativeApp() ? '' : ('<table>' + '  <tbody>' + '    <tr>' + '      <td class="header_left">' + '        <label class="touchable_button">' + '          <input type="submit" value="' + FB.Intl.tx._("Cancel") + '"' + '            id="fb_dialog_loader_close"/>' + '        </label>' + '      </td>' + '      <td class="header_center">' + '        <div>' + FB.Intl.tx._("Loading...") + '</div>' + '      </td>' + '      <td class="header_right">' + '      </td>' + '    </tr>' + '  </tbody>' + '</table>');

		return FB.Dialog.create({
			classes : 'loading' + (FB.UA.iPad() ? ' centered' : ''),
			content : ('<div class="dialog_header">' + chrome + '</div>')
		});
	},
	...
	showLoader : function(cb, width) {
		...
		if (!FB.Dialog._loaderEl) {
			FB.Dialog._loaderEl = FB.Dialog._findRoot(FB.UA.mobile() ? FB.Dialog._createMobileLoader() : FB.Dialog._createWWWLoader(width));
		}
	...
	_centerActive : function(pageInfo) {
		...
		if (pageInfo) {
			top = pageInfo.scrollTop - pageInfo.offsetTop + (pageInfo.clientHeight - height) / 2;
		}
	...
	_handleOrientationChange : function(e) {
		...
		if (FB.UA.iPad()) {
			...
			for (var id in FB.Dialog._dialogs) {
				...
				if (document.getElementById(id)) {
					document.getElementById(id).style.width = FB.UIServer.getDefaultSize().width + 'px';
				}
			}
		}
	},
	...
	_addOrientationHandler : function() {
		...
		var event_name = "onorientationchange" in window ? 'orientationchange' : 'resize';
	...
	create : function(opts) {
		...
		var dialog = document.createElement('div'), contentRoot = document.createElement('div'), className = 'fb_dialog';
		...
		if (FB.UA.ie()) {
			...
			FB.Array.forEach(['vert_left', 'vert_right', 'horiz_top', 'horiz_bottom', 'top_left', 'top_right', 'bottom_left', 'bottom_right'], function(name) {
				var span = document.createElement('span');
				span.className = 'fb_dialog_' + name;
				dialog.appendChild(span);
			});
		} else {
			className += (FB.UA.mobile()) ? ' fb_dialog_mobile' : ' fb_dialog_advanced';
		}
...
FB.provide('', {
	...
	ui : function(params, cb) {
			...
			switch (params.method) {
				case 'auth.login':
					FB._nativeInterface.login(params, cb, function(e) {
						loader("hide");
						if (!e == "Cancelled") {
							alert('Facebook Connect plugin fail on login!' + e);
						}
					});
					break;
				...
				case 'auth.status':
					FB._nativeInterface.getLoginStatus(cb, function(e) {
						alert('facebook Connect plugin fail on auth.status!');
					});
					break;
				case 'login.status':
					FB._nativeInterface.getLoginStatus(cb, function(e) {
		...
		if ((params.method == 'permissions.request' || params.method == 'permissions.oauth') && (params.display == 'iframe' || params.display == 'dialog')) {
			var perms;
			var requested_perms;
			perms = params.scope;
...
FB.provide('UIServer', {
	...
	prepareCall : function(params, cb) {
		var name = params.method.toLowerCase(), method = FB.copy({}, FB.UIServer.Methods[name]), id = FB.guid(),
		...
		forceHTTPS = (method.noHttps !== true) && (FB._https || (name !== 'auth.status' && name != 'login.status'));
		...
		var transform = method.transform ? method.transform : FB.UIServer.genericTransform;
		if (transform) {
			call = transform(call);
		...
		if (!(call.id in FB.UIServer._defaultCb) && !('next' in call.params) && !('redirect_uri' in call.params)) {
			call.params.next = FB.UIServer._xdResult(call.cb, call.id, relation, true // isDefault
			);
		}
		if (relation === 'parent') {
			call.params.channel_url = FB.UIServer._xdChannelHandler(id, 'parent.parent');
		}
	...
	prepareParams : function(call) {
		...
		if (FB.TemplateUI && FB.TemplateUI.supportsTemplate(method, call)) {
			...
			if (!FB.UA.nativeApp() && FB.UIServer.urlTooLongForIE(call.url + '?' + encodedQS)) {
				call.post = true;
			} else if (encodedQS) {
				call.url += '?' + encodedQS;
			}
	...
	getDisplayMode : function(method, params) {
		if (params.display === 'hidden' || params.display === 'none') {
			return params.display;
		}

		if (FB.Canvas.isTabIframe() && params.display !== 'popup') {
			return 'async';
		}
		...
		if (!FB.getAccessToken() && params.display == 'dialog' && !method.loggedOutIframe) {
	...
	popup : function(call) {
		...
		if (call.post) {
			FB.UIServer.setLoadedNode(call, window.open('about:blank', call.id, features), 'popup');
			...
			FB.UIServer.setLoadedNode(call, window.open(call.url, call.id, features), 'popup');
		}
	...
	getLoadedNode : function(call) {
		var id = typeof call == 'object' ? call.id : call, node = FB.UIServer._loadedNodes[id];
	...
	touch : function(call) {
		if (call.params && call.params.in_iframe) {
			...
			FB.Native.onready(function() {
				...
				// pass a window.name
				FB.UIServer.setLoadedNode(call, FB.Native.open(call.url + '#cb=' + call.frameName));
			});
			FB.UIServer._popupMonitor();
	...
	_handleResizeMessage : function(frame, data) {
		...
		FB.Arbiter.inform('resize.ack', data || {}, 'parent.frames[' + node.name + ']', true);
	...
	_popupMonitor : function() {
		...
		for (var id in FB.UIServer._loadedNodes) {
			...
			if (FB.UIServer._loadedNodes.hasOwnProperty(id) && id in FB.UIServer._defaultCb) {
	...
	_xdChannelHandler : function(frame, relation) {
		var forceHTTPS = (FB.UIServer._forceHTTPS && FB.UA.ie() !== 7);
...
FB.provide('Auth', {
	...
	staticAuthCheck : function(lsCb) {
		...
		FB.Content.insertIframe({
			...
			onload : function(iframe) {
				...
				server.postMessage(FB.JSON.stringify({
					method : 'getItem',
	...
	_staticAuthHandler : function(cb, response) {
		if (response && response.data && response.data.status && response.data.status == 'connected') {
			var r;
	...
	setAuthResponse : function(authResponse, status) {
		...
		if (authResponse) {
			...
			if (authResponse.userID) {
				...
				var parsedSignedRequest = FB.Auth.parseSignedRequest(authResponse.signedRequest);
		...
		var login = !FB._userID && authResponse, logout = FB._userID && !authResponse, both = authResponse && FB._userID != userID, authResponseChange = login || logout || both, statusChange = status != FB._userStatus;
	...
	xdHandler : function(cb, frame, target, authResponse, method) {
		return FB.UIServer._xdNextHandler(FB.Auth.xdResponseWrapper(cb, authResponse, method), frame, target, true);
	},
	...
	xdResponseWrapper : function(cb, authResponse, method) {
		return function(params) {
			if (params.access_token) {
				...
				var parsedSignedRequest = FB.Auth.parseSignedRequest(params.signed_request);
				authResponse = {
				...
				if (FB.Cookie.getEnabled()) {
					var expirationTime = authResponse.expiresIn === 0 ? 0// make this a session cookie if it's for offline access
					...
					FB.Cookie.setSignedRequestCookie(params.signed_request, expirationTime, baseDomain);
				}
...
FB.provide('UIServer.Methods', {
	'permissions.oauth' : {
		...
		transform : function(call) {
			...
			var cb = call.cb, id = call.id;
			delete call.cb;
			FB.copy(call.params, {
				client_id : FB._apiKey,
				redirect_uri : FB.URI.resolve(FB.Auth.xdHandler(cb, id, 'opener', FB._authResponse, 'permissions.oauth')),
	...
	'auth.logout' : {
		...
		transform : function(call) {
			if (!FB._apiKey) {
				...
				call.params.next = FB.Auth.xdHandler(call.cb, call.id, 'parent', FB._authResponse);
				return call;
			}
		}
	},
	...
	'login.status' : {
		...
		transform : function(call) {
			...
			FB.copy(call.params, {
				client_id : FB._apiKey,
				redirect_uri : FB.Auth.xdHandler(cb, id, 'parent', FB._authResponse),
				origin : FB.Auth._getContextType(),
...
FB.provide('Cookie', {
	...
	loadMeta : function() {
		...
		cookie = document.cookie.match('\\bfbm_' + FB._apiKey + '=([^;]*)\\b'), meta;
	...
	loadSignedRequest : function() {
		var cookie = document.cookie.match('\\bfbsr_' + FB._apiKey + '=([^;]*)\\b');
		if (!cookie) {
			return null;
		}
	...
	setSignedRequestCookie : function(signed_request_cookie, expiration_time, base_domain) {
		...
		FB.Cookie.setRaw('fbsr_', signed_request_cookie, expiration_time, base_domain);
	},
	...
	setRaw : function(prefix, val, ts, domain) {
		...
		if (domain) {
			...
			document.cookie = prefix + FB._apiKey + '=; expires=Wed, 04 Feb 2004 08:00:00 GMT;';
			// This domain, (will become .example.com)
			document.cookie = prefix + FB._apiKey + '=; expires=Wed, 04 Feb 2004 08:00:00 GMT;' + 'domain=' + location.hostname + ';';
		}

		var expires = new Date(ts).toGMTString();
		document.cookie = prefix + FB._apiKey + '=' + val + (val && ts === 0 ? '' : '; expires=' + expires) + '; path=/' + ( domain ? '; domain=' + domain : '');
	}
});
...
FB.provide('Canvas.Prefetcher', {
	...
	setCollectionMode : function(mode) {
		...
		if (mode != FB.Canvas.Prefetcher.COLLECT_AUTOMATIC && mode != FB.Canvas.Prefetcher.COLLECT_MANUAL) {
			return false;
		}
	...
	_sample : function() {
		...
		if (FB.Canvas.Prefetcher._collectionMode == FB.Canvas.Prefetcher.COLLECT_AUTOMATIC) {
			FB.Array.forEach(resourceFieldsByTag, function(propertyName, tagName) {
				FB.Array.forEach(window.document.getElementsByTagName(tagName), function(tag) {
					if (tag[propertyName]) {
...
FB.provide('', {
	...
	init : function(options) {
		...
		FB._userID = 0;
		// assume unknown or disconnected unless proved otherwise
		...
		if (!options.logging && window.location.toString().indexOf('fb_debug=1') < 0) {
			FB._logging = false;
		}

		FB.XD.init(options.channelUrl);

		if (FB.UA.mobile() && FB.TemplateUI && FB.TemplateData && FB.TemplateData._enabled && options.useCachedDialogs !== false) {
			FB.TemplateUI.init();
		...
		if (FB._apiKey) {
			...
			if (options.authResponse) {
				FB.Auth.setAuthResponse(options.authResponse, 'connected');
			} else {
				...
				FB._userID = (parsedSignedRequest && parsedSignedRequest.user_id) || 0;
				FB.Cookie.loadMeta();
			}
...
FB.provide('UIServer.MobileIframableMethod', {
	transform : function(call) {
		...
		// access token
		if (call.params.display === 'touch' && call.params.access_token && window.postMessage) {
			// Prepare for iframe by adding an channel parameter
			// for resizing and add in_iframe parameter
			call.params.channel = FB.UIServer._xdChannelHandler(call.id, 'parent');
...
FB.provide('UIServer.Methods', {
	...
	'auth.logintofacebook' : {
		...
		transform : function(call) {
			...
			var next = FB.UIServer._xdResult(call.cb, call.id, relation, true // isDefault
			);
			call.params.next = FB.getDomain(FB._https ? 'https_www' : 'www') + "login.php?" + FB.QS.encode({
				api_key : FB._apiKey,
				next : next,
				skip_api_login : 1
			});
	...
	'apprequests' : {
		transform : function(call) {
			...
			call.params.frictionless = FB.Frictionless && FB.Frictionless._useFrictionless;
			if (call.params.frictionless) {
				...
				call.cb = FB.Frictionless._processRequestResponse(call.cb, call.hideLoader);
			}
			return call;
		},
		getXdRelation : function(params) {
...
FB.provide('XFBML', {
	...
	getElements : function(dom, xmlns, localName) {
		var arr = FB.Array, xfbmlDoms = FB.XFBML._getDomElements(dom, xmlns, localName), html5Doms = FB.Dom.getByClass(xmlns + '-' + localName, dom, 'div');
		...
		html5Doms = arr.filter(html5Doms, function(el) {
			...
			return !el.hasChildNodes() || (el.childNodes.length === 1 && el.childNodes[0].nodeType === 3);
		});
	...
	parse : function(dom, cb) {
		...
		var count = 1, onTagDone = function() {
			count--;
			if (count === 0) {
		...
		FB.Array.forEach(FB.XFBML._tagInfos, function(tagInfo) {
			...
			if (cachedDomElements[tagInfo.localName] !== undefined) {
				...
				xfbmlDoms = FB.XFBML.getElements(dom, tagInfo.xmlns, tagInfo.localName);
			}
		...
		window.setTimeout(function() {
			if (count > 0) {
				FB.log(count + ' XFBML tags failed to render in ' + FB.XFBML._renderTimeout + 'ms.');
			}
	...
	shouldUseWidgetPipe : function() {
		...
		var aboveThreshold = FB.XFBML._widgetPipeEnabledTagCount > 1;
	...
	getAttr : function(el, attr) {
		return (el.getAttribute(attr) || el.getAttribute(attr.replace(/_/g, '-')) || el.getAttribute(attr.replace(/-/g, '_')) || el.getAttribute(attr.replace(/-/g, '')) || el.getAttribute(attr.replace(/_/g, '')) || el.getAttribute('data-' + attr) || el.getAttribute('data-' + attr.replace(/_/g, '-')) || el.getAttribute('data-' + attr.replace(/-/g, '_')) || el.getAttribute('data-' + attr.replace(/-/g, '')) || el.getAttribute('data-' + attr.replace(/_/g, '')) || null
		);
	},
...
FB.subclass('Data.Query', 'Waitable', function() {
	 ...
	 */
	parse : function(args) {
		var fql = FB.String.format.apply(null, args), re = (/^select (.*?) from (\w+)\s+where (.*)$/i).exec(fql);
		// Parse it
	 ...
	 */
	toFql : function() {
		var s = 'select ' + this.fields.join(',') + ' from ' + this.table + ' where ';
		switch (this.where.type) {
			...
			case 'in':
				if (this.where.value.length == 1) {
					...
					s += this.where.key + ' in (' + FB.Array.map(this.where.value, this._encode).join(',') + ')';
				}
				break;
		}
		return s;
...
FB.provide('Data', {
	...
	_process : function() {
		...
		FB.api('/fql', 'GET', params, function(result) {
			if (result.error) {
	...
	_mergeIndexQuery : function(item, mqueries) {
		var key = item.where.key, value = item.where.value;
	...
	onready : function(func) {
		...
		if (this.nativeReady) {
			...
			var nativeReadyCallback = function(evt) {
				window.removeEventListener(FB.Native.NATIVE_READY_EVENT, nativeReadyCallback);
				FB.Native.onready(func);
			};
			window.addEventListener(FB.Native.NATIVE_READY_EVENT, nativeReadyCallback, false);
		}
	}
});
...
FB.provide('Helper', {
	 ...
	 */
	isUser : function(id) {
		return id < 2200000000 || (id >= 100000000000000 && // 100T is first 64-bit UID
	...
	getLoggedInUser : function() {
		return FB.getUserID();
		// pass the buck to the auth response
	},
	...
	getProfileLink : function(userInfo, html, href) {
		href = href || ( userInfo ? FB.getDomain('www') + 'profile.php?id=' + userInfo.uid : null);
		if (href) {
	...
	fireEvent : function(eventName, eventSource) {
		...
		FB.Event.fire(eventName, href, eventSource);
		// global
	},
...
FB.provide('TemplateData', {
	...
	_isStale : function(response) {
		if (!response || !response.version || response.version != FB.TemplateData._version || response.currentUserID != FB.getUserID()) {
			return true;
		}
		var currentTime = Math.round((new Date()).getTime());
		return (currentTime - response.setAt) / 1000.0 > FB.TemplateData._localStorageTimeout;
	},

	getResponse : function() {
		var self = FB.TemplateData;
		try {
			self._response = self._response || (self.supportsLocalStorage() && FB.JSON.parse(localStorage.FB_templateDataResponse || "null"));
		} catch (e) {
...
// Instance Methods
{
	...
	use : function(call) {
		...
		FB.UIServer.setLoadedNode(call, FB.UIServer.getLoadedNode(this.cachedCall.id));
		...
		template_params.frictionless = FB.TemplateUI.isFrictionlessAppRequest(this.method, template_params);
	...
	setParams : function(params) {
		...
		this.monitor('getParamsCb', this.bind(function() {
			if (this.getParamsCb) {
				var dialogWindow = frames[this.cachedCall.frameName] || FB.UIServer.getLoadedNode(this.cachedCall);
...
FB.provide('TemplateUI', {
	...
	supportsTemplate : function(method, call) {
		return FB.TemplateData.enabled() && FB.TemplateUI.paramsAllowTemplate(method, call.params) && call.params.display === 'touch' && FB.UA.mobile();
	},
	...
	isFrictionlessAppRequest : function(method, app_params) {
		return method === 'apprequests' && FB.Frictionless && FB.Frictionless._useFrictionless;
	},
	...
	willWriteOnGet : function(method, app_params) {
		return FB.TemplateUI.isFrictionlessAppRequest(method, app_params) && app_params.to && FB.Frictionless.isAllowed(app_params.to);
	}
});
...
FB.provide('URI', {
	...
	resolve : function(uri) {
		...
		// This will be an absolute URL.  MAGIC!
	}
});
...
FB.subclass('XFBML.IframeWidget', 'XFBML.Element', null, {
	...
	arbiterInform : function(event, message, behavior) {
		if (this.loaded) {
			...
			'iframe.onload', FB.bind(this._arbiterInform, this, event, message, behavior));
		}
	},
	...
	_arbiterInform : function(event, message, behavior) {
		...
		FB.Arbiter.inform(event, message, relation, window.location.protocol == 'https:', behavior);
	},
	 ...
	 */
	_getURL : function() {
		var domain = this.getDefaultWebDomain(), static_path = '';
		...
		return FB.getDomain(domain) + 'plugins/' + static_path + this.getUrlBits().name + '.php';
	},
...
FB.provide('XFBML.IframeWidget', {
	...
	batchWidgetPipeRequests : function() {
		...
		var widgetPipeDescriptions = FB.XFBML.IframeWidget._groupWidgetPipeDescriptions();
		...
		var masterWidgetPipeSpan = masterWidgetPipeDom.appendChild(document.createElement('span'));
		FB.Content.insertIframe({
			url : 'about:blank',
...
FB.subclass('XFBML.ButtonElement', 'XFBML.Element', null, {
	...
	process : function() {
		...
		var size = this._getAttributeFromList('size', 'medium', this._allowedSizes), className = '', markup = '';
		...
		if (markup !== '') {
			this.dom.innerHTML = ('<a class="' + className + '">' + '<span class="fb_button_text">' + markup + '</span>' + '</a>'
			);
...
FB.subclass('XFBML.Comments', 'XFBML.IframeWidget', null, {
	...
	setupAndValidate : function() {
		...
		if (FB.initSitevars.enableMobileComments && FB.UA.mobile() && attr.mobile !== false) {
			attr.mobile = true;
		}
		...
		if (!attr.href) {
			...
			if (attr.migrated) {
				attr.href = 'http://www.facebook.com/plugins/comments_v1.php?' + 'app_id=' + FB._apiKey + '&xid=' + encodeURIComponent(attr.xid) + '&url=' + encodeURIComponent(attr.url);
			}
		} else {
			// allows deep linking of comments by surfacing linked comments
			var fb_comment_id = this.getAttribute('fb_comment_id');
			if (!fb_comment_id) {
				fb_comment_id = FB.QS.decode(document.URL.substring(document.URL.indexOf('?') + 1)).fb_comment_id;
...
FB.subclass('XFBML.CommentsCount', 'XFBML.Element', null, {
	...
	process : function() {
		...
		this._count.wait(FB.bind(function() {
			...
			this.dom.innerHTML = FB.String.format('<span class="fb_comments_count">{0}</span>', c);
...
FB.provide('Anim', {
	...
	ate : function(dom, props, duration, callback) {
		...
		var frame_speed = 40, from = {}, to = {}, begin = null, s = dom.style, timer = setInterval(FB.bind(function() {
			if (!begin) {
			...
			FB.Array.forEach(props, FB.bind(function(value, prop) {
				...
				FB.Array.forEach(from[prop], function(pair, i) {
					...
					if (isNaN(to[prop][i].numPart) && to[prop][i].textPart == '?') {
						...
						next += (pair.numPart + // orig value
						Math.ceil((to[prop][i].numPart - pair.numPart) * Math.sin(Math.PI / 2 * pd))) + to[prop][i].textPart + ' ';
						// text part and trailing space
					}
				});
...
FB.provide('Insights', {
	...
	impression : function(params, cb) {
		...
		var g = FB.guid(), u = "//ah8.facebook.com/impression.php/" + g + "/", i = new Image(1, 1), s = [];
...
FB.subclass('XFBML.ConnectBar', 'XFBML.Element', null, {
	 ...
	 */
	_showBar : function() {
		var q1 = FB.Data._selectByIndex(['first_name', 'profile_url', this._picFieldName], 'user', 'uid', this._uid);
		var q2 = FB.Data._selectByIndex(['display_name'], 'application', 'api_key', FB._apiKey);
	...
	_renderConnectBar : function(info) {
		var bar = document.createElement('div'), container = document.createElement('div');
		...
		this._initialHeight = Math.round(parseFloat(FB.Dom.getStyle(container, 'height')) + parseFloat(FB.Dom.getStyle(container, 'borderBottomWidth')));
		...
		}), FB.Intl.tx._("Learn More"), info.profile_url, FB.getDomain('www') + 'sitetour/connect.php');
		var _this = this;
		...
		if (this._page.parentNode) {
			top_margin = Math.round((parseFloat(FB.Dom.getStyle(this._page.parentNode, 'height')) - parseFloat(FB.Dom.getStyle(this._page, 'height'))) / 2);
		} else {
 ...
 * @private
 */
FB.subclass('XFBML.EdgeCommentWidget', 'XFBML.IframeWidget', function(opts) {
...
FB.subclass('XFBML.EdgeWidget', 'XFBML.IframeWidget', null, {
	...
	_getWidgetWidth : function() {
		...
		var standard_min_width = ( recommend ? 265 : 225) + ( send ? 60 : 0);
		var button_count_default_width = ( recommend ? 130 : 90) + ( send ? 60 : 0);
		var box_count_default_width = this.getAttribute('action') === 'recommend' ? 100 : 55;
		var simple_default_width = this.getAttribute('action') === 'recommend' ? 90 : 50;
	...
	_createEdgeCommentWidget : function(message, comment_node) {
		var opts = {
			...
			anchorTargetX : parseFloat(message['query[anchorTargetX]']) + this._rootPadding.left,
			anchorTargetY : parseFloat(message['query[anchorTargetY]']) + this._rootPadding.top,
	...
	_getHeightOffset : function(message) {
		return parseFloat(message['anchorGeometry[y]']) + parseFloat(message['anchorPosition[y]']) + this._rootPadding.top;
	},
	...
	_fireEventAndInvokeHandler : function(eventName, eventAttribute) {
		...
		FB.Helper.invokeHandler(this.getAttribute(eventAttribute), this, [this._attr.href]);
		// inline
	},
 ...
 * @private
 */
FB.subclass('XFBML.SendButtonFormWidget', 'XFBML.EdgeCommentWidget', function(opts) {
	this._base(opts);

	FB.Dom.addCss(this.dom, 'fb_send_button_form_widget');
	FB.Dom.addCss(this.dom, opts.colorscheme);
	FB.Dom.addCss(this.dom, ( typeof opts.siderender != 'undefined' && opts.siderender) ? 'siderender' : '');
...
FB.subclass('XFBML.Like', 'XFBML.EdgeWidget', null, {
	...
	_createEdgeCommentWidget : function(message, comment_node) {
		...
		// send="true"></fb:like>)
		if ('send' in this._attr && 'widget_type' in message && message.widget_type == 'send') {
			var opts = this._getCommonEdgeCommentWidgetOpts(message, comment_node);
			return new FB.XFBML.SendButtonFormWidget(opts);
		} else {
			return this._callBase("_createEdgeCommentWidget", message, comment_node);
		}
	},
...
FB.subclass('XFBML.LoginButton', 'XFBML.ButtonElement', null, {
	...
	setupAndValidate : function() {
		...
		this._attr = {
			...
			length : this._getAttributeFromList('length', // name
			'short', // defaultValue
			['long', 'short'] // allowed
			),
		...
		if (this._attr.registration_url) {
			...
			FB.Event.subscribe('auth.statusChange', this._saveStatus(this.process, /* on_click */false));
			...
			switch (this._attr.status) {
				case 'unknown':
					FB.ui({
						...
						FB.bind(FB.getLoginStatus(this._saveStatus(this._authCallback, /* on_click */true),
						/* force */true), this);
					}, this));
					break;
				case 'notConnected':
	...
	_saveStatus : function(cb, on_click) {
		return FB.bind(function(response) {
			if (on_click && this._attr.registration_url && (this._attr.status == 'notConnected' || this._attr.status == 'not_authorized') && (response.status == 'notConnected' || response.status == 'not_authorized')) {
...
FB.subclass('XFBML.Name', 'XFBML.Element', null, {
	...
	_renderPronoun : function(userInfo) {
		var word = '', objective = this._objective;
		if (this._subjectId) {
			objective = true;
			if (this._subjectId === this._uid) {
				this._reflexive = true;
	...
	_renderOther : function(userInfo) {
		...
		if (!name) {
			name = FB.String.escapeHTML(this.getAttribute('if-cant-see', 'Facebook User'));
		}
		if (name) {
			if (this._getBoolAttribute('capitalize', false)) {
			...
			if (userInfo && this._linked) {
				html = FB.Helper.getProfileLink(userInfo, name, this.getAttribute('href', null));
			} else {
				html = name;
			}
		}
...
FB.subclass('XFBML.ProfilePic', 'XFBML.Element', null, {
	 ...
	 */
	process : function() {
		var size = this.getAttribute('size', 'thumb'), picFieldName = FB.XFBML.ProfilePic._sizeToPicFieldMap[size], width = this._getPxAttribute('width'), height = this._getPxAttribute('height'), style = this.dom.style, uid = this.getAttribute('uid');
		...
		var renderFn = this.bind(function(result) {
			var userInfo = result ? result[0] : null, imgSrc = userInfo ? userInfo[picFieldName] : null;

			if (!imgSrc) {
				// Create default
				imgSrc = FB.getDomain('cdn') + FB.XFBML.ProfilePic._defPicMap[picFieldName];
			}
			// Copy width, height style, and class name of fb:profile-pic down to the
			// image element we create
			var styleValue = (( width ? 'width:' + width + ';' : '') + ( height ? 'height:' + width + ';' : '')
			), html = FB.String.format('<img src="{0}" alt="{1}" title="{1}" style="{2}" class="{3}" />', imgSrc, userInfo ? FB.String.escapeHTML(userInfo.name) : '', styleValue, this.dom.className);

			if (this._getBoolAttribute('linked', true)) {
				html = FB.Helper.getProfileLink(userInfo, html, this.getAttribute('href', null));
			}
			this.dom.innerHTML = html;
		...
		FB.Event.monitor('auth.statusChange', this.bind(function() {
			...
			if (FB._userStatus && uid) {
				...
				FB.Data._selectByIndex(['name', picFieldName], FB.Helper.isUser(uid) ? 'user' : 'profile', FB.Helper.isUser(uid) ? 'uid' : 'id', uid).wait(renderFn);
			} else {
				// Render default
				renderFn();
			}
...
FB.subclass('XFBML.RecommendationsBar', 'XFBML.IframeWidget', null, {
	...
	setupAndValidate : function() {
		...
		if (this._attr.trigger == "manual") {
			var manual = FB.bind(function(href) {
			...
			var trigger = interval_queue(500, FB.bind(function() {
			...
			trigger();
			// in case <fb:recommendations-bar /> is already visible
		}

		this.visible = false;
		var visible = interval_queue(500, FB.bind(function() {
		...
		this.focused = true;
		var toggleFocused = FB.bind(function() {
	...
	_checkNextResize : function() {
		...
		if (this.animate) {
			this.animate = false;
			// the signal causes one resize to animate
	...
	setupAndValidate : function() {
		...
		if (this._attr.onvalidate) {
			this.subscribe('xd.validate', this.bind(function(message) {
				...
				// Call their function
				var response = FB.Helper.executeFunctionByName(this._attr.onvalidate, value, callback);
...
FB.subclass('XFBML.ShareButton', 'XFBML.Element', null, {
	...
	_renderButton : function(skipRenderEvent) {
		...
		var contentStr = '', post = '', pre = '', classStr = '', share = FB.Intl.tx._("Share"), wrapperClass = '';

		switch (this._type) {
			case 'icon':
			case 'icon_link':
				classStr = 'fb_button_simple';
				contentStr = ('<span class="fb_button_text">' + (this._type == 'icon_link' ? share : '&nbsp;') + '</span>'
				);
			...
			case 'button_count':
				contentStr = '<span class="fb_button_text">' + share + '</span>';
				post = ('<span class="fb_share_count_nub_right">&nbsp;</span>' + '<span class="fb_share_count fb_share_count_right">' + this._getCounterMarkup() + '</span>'
				);
				classStr = 'fb_button fb_button_small';
				break;
			default:
				// box count
				contentStr = '<span class="fb_button_text">' + share + '</span>';
				pre = ('<span class="fb_share_count_nub_top">&nbsp;</span>' + '<span class="fb_share_count fb_share_count_top">' + this._getCounterMarkup() + '</span>'
				);
				classStr = 'fb_button fb_button_small';
		...
		this.dom.innerHTML = FB.String.format('<span class="{0}">{4}<a id="{1}" class="{2}" ' + 'target="_blank">{3}</a>{5}</span>', wrapperClass, a_id, classStr, contentStr, pre, post);
		...
		if (this._count.value !== undefined) {
			if (this._count.value.length > 0) {
				...
				if (c > 3) {
					...
					prettyCount = c >= 10000000 ? Math.round(c / 1000000) + 'M' : (c >= 10000 ? Math.round(c / 1000) + 'K' : c);
				}
			}
		} else {
