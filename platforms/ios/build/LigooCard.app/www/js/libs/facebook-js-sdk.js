  ...
  ui: function(params, cb) {
    ...
    if (FB._nativeInterface) {
        switch (params.method) {
            case 'auth.login':
                FB._nativeInterface.login(params, cb, function(e) {loader("hide"); alert('Facebook Connect plugin fail on login! ' + e);});
                break;
            ...
            case 'permissions.oauth':
                ...
                break;
            case 'auth.logout':
                FB._nativeInterface.logout(cb, function(e) {alert('Facebook Connect plugin fail on logout!');});
                break;
            case 'auth.status':
                FB._nativeInterface.getLoginStatus(cb, function(e) {alert('facebook Connect plugin fail on auth.status!');});
                break;
            case 'login.status':
                FB._nativeInterface.getLoginStatus(cb, function(e) {alert('Facebook Connect plugin fail on auth.status!');});
                break;
            case 'feed':
                FB._nativeInterface.dialog(params, cb, function(e) {alert('Facebook Connect plugin fail on auth.status!');});
                break;
            case 'apprequests':
                FB._nativeInterface.dialog(params, cb, function(e) {alert('Facebook Connect plugin fail on auth.status!');});
            break;
